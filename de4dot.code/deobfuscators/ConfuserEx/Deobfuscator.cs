		...
		private class Deobfuscator : DeobfuscatorBase
		{
				...
				if (_constantDecrypter.Detected)
				{
					...
					foreach (var info in _constantDecrypter.Decrypters)
					{
						staticStringInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptString(info, gim, args[0]));
						_sbyteValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptSByte(info, gim, args[0]));
						_byteValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptByte(info, gim, args[0]));
						_int16ValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptInt16(info, gim, args[0]));
						_uint16ValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptUInt16(info, gim, args[0]));
						_int32ValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptInt32(info, gim, args[0]));
						_uint32ValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptUInt32(info, gim, args[0]));
						_int64ValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptInt64(info, gim, args[0]));
						_uint64ValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptUInt64(info, gim, args[0]));
						_singleValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptSingle(info, gim, args[0]));
						_doubleValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptDouble(info, gim, args[0]));
						_arrayValueInliner.Add(info.Method,
							(method, gim, args) => _constantDecrypter.DecryptArray(info, gim, args[0]));
					}
					_deobfuscating = true;
				}
			...
			public override void DeobfuscateEnd()
			{
				...
				if (_constantDecrypter.Detected)
					...
					{
						if (!_constantDecrypter.MethodIsInlined) {
